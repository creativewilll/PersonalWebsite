  ...
  return (
    <section ref={containerRef} className="relative w-full min-h-[100dvh] flex items-center justify-center overflow-hidden pt-24 pb-12">
      ...
      <motion.div style={{ y, opacity }} className="absolute inset-0 z-0 bg-[var(--color-bg)]">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(139,92,246,0.15),rgba(255,255,255,0))] object-cover" />
        <div 
          className="absolute inset-0 opacity-20 mix-blend-screen bg-cover bg-center"
          style={{ backgroundImage: "url('https://picsum.photos/seed/concertAI/1920/1080')" }}
        />
        {/* Animated gradients for spotlight effect */}
        <div className="absolute top-1/4 left-1/4 w-[40vw] h-[40vw] bg-[var(--color-secondary)]/30 rounded-full blur-[120px] animate-pulse-border" style={{ animationDuration: "10s" }} />
        <div className="absolute bottom-10 right-1/4 w-[35vw] h-[35vw] bg-[var(--color-primary)]/20 rounded-full blur-[150px] animate-pulse-border" style={{ animationDuration: "15s" }} />
      </motion.div>
      
      <div className="noise-overlay" />

      <div className="relative z-10 w-full max-w-[1000px] mx-auto px-6 flex flex-col items-center text-center mt-10">
        <motion.div
        ...
        <h1 className="font-display font-black text-[clamp(2.5rem,6vw,7.5rem)] leading-[0.95] tracking-tight mb-8 text-[var(--color-text)] relative z-10 w-full">
          <RevealText delay={0.1}>Your Music Deserves</RevealText><br/>
          <RevealText delay={0.3}>a Website That</RevealText><br/>
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-[var(--color-primary)] via-[var(--color-secondary)] to-[var(--color-tertiary)] inline-block pb-4">
            <RevealText delay={0.5}>Actually Works.</RevealText>
          </span>
        </h1>
        ...
        >
          Custom websites, merch stores, print & fulfillment, Stripe + PayPal payments, Laylo fan drops, and relentless SEO growth — all handled for you, so you can focus on the <span className="text-[var(--color-text)] font-medium">music</span>.
        </motion.p>
